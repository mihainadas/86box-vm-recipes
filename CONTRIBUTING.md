...
The named suites `repository-safety`, `boot-floppy`, `software-media`, and `macos-launcher` are useful while iterating on one area; for example, run `scripts/check.sh software-media` after changing the catalog, a bundle, or the media builder. An explicitly selected suite treats its core prerequisites as required. Keep configuration host-neutral, document unusual values, and test the recipe from a clean directory when practical.
...
Do not commit operating systems, product keys, installed disk images, ROMs, NVR state, firmware dumps, proprietary drivers, or media copied from another project. A new `redistributable` software-catalog entry requires review of the exact artifact, official HTTPS provenance, byte count, SHA-256, license basis, source obligations, and notices. Use `reference-only` or `user-supplied` whenever those rights are unclear. Tests must use synthetic bytes only.
